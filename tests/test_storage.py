...
class TestArticleOperations:
    ...
    def test_store_article_returns_nanoid_format(self, initialized_db):
        ...
        from src.storage.sqlite import store_article, add_feed
    ...
    def test_store_article_async_returns_nanoid(self, initialized_db):
        ...
        import asyncio
        from src.storage.sqlite import store_article_async, add_feed
        from src.models import Feed
    ...
    def test_list_articles_returns_ordered_by_pub_date(self, initialized_db):
        ...
        from src.storage.sqlite import store_article, add_feed, list_articles
        from src.models import Feed
    ...
    def test_get_article_returns_single_article(self, initialized_db):
        ...
        from src.storage.sqlite import store_article, add_feed, get_article
        from src.models import Feed
    ...
    def test_get_article_detail_returns_dict_with_tags(self, initialized_db):
        """get_article_detail() returns dict with all fields including tags."""
        from src.storage.sqlite import store_article, add_feed, get_article_detail, tag_article
        from src.models import Feed

        feed = Feed(
            id="feed-detail",
            name="Detail Feed",
            url="https://example.com/detail.xml",
            etag=None,
            last_modified=None,
            last_fetched=None,
            created_at="2024-01-01T00:00:00+00:00",
        ...
        tag_article(article_id, "python")
        tag_article(article_id, "testing")
        ...
        assert detail["content"] == "<p>Full content here</p>"
        assert detail["feed_name"] == "Detail Feed"
        assert "tags" in detail
        assert "python" in detail["tags"]
        assert "testing" in detail["tags"]
    ...
    def test_get_article_detail_with_truncated_8char_id(self, initialized_db):
        ...
        from src.storage.sqlite import store_article, add_feed, get_article_detail
        from src.models import Feed
    ...
    def test_search_articles_fts5_returns_matches(self, initialized_db):
        ...
        from src.storage.sqlite import store_article, add_feed, search_articles
        from src.models import Feed
    ...
    def test_list_articles_with_tags_single_tag(self, initialized_db):
        """list_articles_with_tags() returns articles with specific tag."""
        from src.storage.sqlite import store_article, add_feed, tag_article, list_articles_with_tags
        from src.models import Feed

        feed = Feed(
            id="feed-tag",
            name="Tag Feed",
            url="https://example.com/tag.xml",
            etag=None,
            last_modified=None,
            last_fetched=None,
            created_at="2024-01-01T00:00:00+00:00",
        ...
        article1_id = store_article(
            guid="tagged-article-1",
        ...
        article2_id = store_article(
            guid="tagged-article-2",
            title="Tagged Article 2",
            content="Content 2",
            link="https://example.com/tag2",
            feed_id="feed-tag",
            pub_date="2024-01-14T10:00:00+00:00",
        )
        store_article(
            guid="untagged-article",
            title="Untagged Article",
            content="Content 3",
            link="https://example.com/untagged",
            feed_id="feed-tag",
            pub_date="2024-01-13T10:00:00+00:00",
        )

        tag_article(article1_id, "important")
        tag_article(article2_id, "important")
        tag_article(article2_id, "python")

        results = list_articles_with_tags(tag="important")

        assert len(results) == 2
        titles = [r.title for r in results]
        assert "Tagged Article 1" in titles
        assert "Tagged Article 2" in titles

    def test_list_articles_with_tags_multiple_tags_or_logic(self, initialized_db):
        """list_articles_with_tags() with tags param uses OR logic (has a OR has b)."""
        from src.storage.sqlite import store_article, add_feed, tag_article, list_articles_with_tags
        from src.models import Feed
        ...
        feed = Feed(
            id="feed-tags-or",
            name="Tags OR Feed",
            url="https://example.com/tags-or.xml",
            etag=None,
            last_modified=None,
            last_fetched=None,
            created_at="2024-01-01T00:00:00+00:00",
        ...
        article1_id = store_article(
            ...
            title="OR Article 1",
            content="Content 1",
            link="https://example.com/or1",
            feed_id="feed-tags-or",
            pub_date="2024-01-15T10:00:00+00:00",
        )
        article2_id = store_article(
            guid="or-article-2",
            title="OR Article 2",
            content="Content 2",
            link="https://example.com/or2",
            feed_id="feed-tags-or",
            pub_date="2024-01-14T10:00:00+00:00",
        )
        article3_id = store_article(
            guid="or-article-3",
            title="OR Article 3",
            content="Content 3",
            link="https://example.com/or3",
            feed_id="feed-tags-or",
            pub_date="2024-01-13T10:00:00+00:00",
        )

        tag_article(article1_id, "python")
        tag_article(article2_id, "java")
        # article3 has no tags

        # OR logic: articles with "python" OR "java"
        results = list_articles_with_tags(tags="python,java")

        assert len(results) == 2
        titles = [r.title for r in results]
        assert "OR Article 1" in titles
        assert "OR Article 2" in titles
        assert "OR Article 3" not in titles

    def test_get_articles_with_tags_batch_fetch(self, initialized_db):
        """get_articles_with_tags() batch fetches tags for multiple article IDs."""
        from src.storage.sqlite import store_article, add_feed, tag_article, get_articles_with_tags
        from src.models import Feed
        ...
        tag_article(article1_id, "python")
        tag_article(article1_id, "testing")
        tag_article(article2_id, "python")

        result = get_articles_with_tags([article1_id, article2_id])

        assert isinstance(result, dict)
        assert article1_id in result
        assert article2_id in result
        assert "python" in result[article1_id]
        assert "testing" in result[article1_id]
        assert "python" in result[article2_id]
        assert "testing" not in result[article2_id]

    def test_get_articles_with_tags_empty_list(self, initialized_db):
        """get_articles_with_tags() with empty list returns empty dict."""
        from src.storage.sqlite import get_articles_with_tags

        result = get_articles_with_tags([])
        assert result == {}
...
class TestFeedOperations:
    ...
    def test_feed_exists_with_existing_url(self, initialized_db):
        ...
        from src.storage.sqlite import feed_exists, add_feed
        from src.models import Feed
    ...
    def test_add_feed_inserts_and_returns_feed(self, initialized_db):
        ...
        feed = Feed(
            ...
            last_fetched="2024-01-15T10:00:00+00:00",
            created_at="2024-01-01T00:00:00+00:00",
        )
    ...
    def test_list_feeds_returns_with_articles_count(self, initialized_db):
        ...
        from src.storage.sqlite import add_feed, store_article, list_feeds
        from src.models import Feed
    ...
    def test_remove_feed_with_existing_id(self, initialized_db):
        ...
        from src.storage.sqlite import add_feed, remove_feed, get_feed, list_feeds
        from src.models import Feed
    ...
    def test_remove_feed_cascade_deletes_articles(self, initialized_db):
        ...
        from src.storage.sqlite import add_feed, store_article, remove_feed, list_feeds
        from src.models import Feed
...
class TestTagOperations:
    """Tests for tag storage functions: add_tag, list_tags, remove_tag,
    get_tag_article_counts, tag_article, untag_article, get_article_tags.
    """

    def test_add_tag_creates_tag_returns_nanoid(self, initialized_db):
        """add_tag() creates tag and returns Tag object with nanoid id."""
        from src.storage.sqlite import add_tag, list_tags
    ...
    def test_add_tag_duplicate_name_raises_integrity_error(self, initialized_db):
        """add_tag() with duplicate name raises sqlite3.IntegrityError."""
        from src.storage.sqlite import add_tag

        add_tag("duplicate-tag")

        with pytest.raises(sqlite3.IntegrityError):
            add_tag("duplicate-tag")

    def test_list_tags_ordered_by_name(self, initialized_db):
        """list_tags() returns list ordered by name."""
        from src.storage.sqlite import add_tag, list_tags
    ...
    def test_list_tags_empty_returns_empty_list(self, initialized_db):
        """list_tags() with no tags returns empty list."""
        from src.storage.sqlite import list_tags

        tags = list_tags()
        assert tags == []

    def test_remove_tag_with_existing_name(self, initialized_db):
        """remove_tag() with existing name returns True and tag is deleted."""
        from src.storage.sqlite import add_tag, remove_tag, list_tags

        add_tag("to-remove")

        result = remove_tag("to-remove")

        assert result is True
        tags = list_tags()
        assert len(tags) == 0

    def test_remove_tag_with_non_existing_name(self, initialized_db):
        """remove_tag() with non-existing name returns False."""
        from src.storage.sqlite import remove_tag

        result = remove_tag("non-existent-tag")
        assert result is False

    def test_remove_tag_cascade_unlinks_from_articles(self, initialized_db):
        """remove_tag() unlinks tag from articles (FK CASCADE)."""
        from src.storage.sqlite import add_tag, add_feed, store_article, tag_article, remove_tag, get_article_tags
        from src.models import Feed

        feed = Feed(
            id="tag-cascade-feed",
            name="Tag Cascade Feed",
            url="https://example.com/tag-cascade.xml",
            etag=None,
            last_modified=None,
            last_fetched=None,
            created_at="2024-01-01T00:00:00+00:00",
        ...
        article_id = store_article(
            guid="cascade-tag-guid",
            title="Cascade Tag Article",
            content="Content",
            link="https://example.com/ct",
            feed_id="tag-cascade-feed",
            pub_date="2024-01-15T10:00:00+00:00",
        )

        tag_article(article_id, "to-cascade")
        assert "to-cascade" in get_article_tags(article_id)

        remove_tag("to-cascade")

        # Tag should be unlinked
        assert get_article_tags(article_id) == []

    def test_get_tag_article_counts_returns_dict(self, initialized_db):
        """get_tag_article_counts() returns {tag_name: count} for all tags."""
        from src.storage.sqlite import add_feed, store_article, tag_article, get_tag_article_counts
        from src.models import Feed

        feed = Feed(
            id="counts-feed",
            name="Counts Feed",
            url="https://example.com/counts.xml",
            etag=None,
            last_modified=None,
            last_fetched=None,
            created_at="2024-01-01T00:00:00+00:00",
        ...
        article1_id = store_article(
            guid="counts-article-1",
            title="Counts Article 1",
            content="Content 1",
            link="https://example.com/c1",
            feed_id="counts-feed",
            pub_date="2024-01-15T10:00:00+00:00",
        ...
        article2_id = store_article(
            guid="counts-article-2",
            title="Counts Article 2",
            content="Content 2",
            link="https://example.com/c2",
            feed_id="counts-feed",
            pub_date="2024-01-16T10:00:00+00:00",
    ...
    def test_tag_article_links_and_creates_tag_if_not_exists(self, initialized_db):
        """tag_article() links article to tag, creates tag if not exists, returns True."""
        from src.storage.sqlite import add_feed, store_article, tag_article, get_article_tags, list_tags
        from src.models import Feed

        feed = Feed(
            id="tag-feed",
            name="Tag Article Feed",
            url="https://example.com/tag-article.xml",
            etag=None,
            last_modified=None,
            last_fetched=None,
            created_at="2024-01-01T00:00:00+00:00",
        ...
        # Tag with a new tag name (should create it)
        result = tag_article(article_id, "new-tag")

        assert result is True
        assert "new-tag" in get_article_tags(article_id)
        # Tag should be created
        tags = list_tags()
        tag_names = [t.name for t in tags]
        assert "new-tag" in tag_names

    def test_tag_article_duplicate_link_returns_true(self, initialized_db):
        """tag_article() with already linked article returns True (no error)."""
        from src.storage.sqlite import add_feed, store_article, tag_article
        from src.models import Feed

        feed = Feed(
            id="dup-link-feed",
            name="Dup Link Feed",
            url="https://example.com/dup-link.xml",
            etag=None,
            last_modified=None,
            last_fetched=None,
            created_at="2024-01-01T00:00:00+00:00",
        ...
        article_id = store_article(
            ...
            feed_id="dup-link-feed",
            pub_date="2024-01-15T10:00:00+00:00",
        )

        tag_article(article_id, "dup-tag")
        # Tag again - should not raise
        result = tag_article(article_id, "dup-tag")

        assert result is True

    def test_untag_article_with_linked_tag(self, initialized_db):
        """untag_article() with linked tag returns True and link is removed."""
        from src.storage.sqlite import add_feed, store_article, tag_article, untag_article, get_article_tags
        from src.models import Feed

        feed = Feed(
            id="untag-feed",
            name="Untag Feed",
            url="https://example.com/untag.xml",
            etag=None,
            last_modified=None,
            last_fetched=None,
            created_at="2024-01-01T00:00:00+00:00",
        ...
        article_id = store_article(
            ...
            title="Untag Article",
            content="Content",
            link="https://example.com/ut",
            feed_id="untag-feed",
            pub_date="2024-01-15T10:00:00+00:00",
        )

        tag_article(article_id, "to-untag")
        assert "to-untag" in get_article_tags(article_id)

        result = untag_article(article_id, "to-untag")

        assert result is True
        assert "to-untag" not in get_article_tags(article_id)

    def test_untag_article_with_non_linked_tag(self, initialized_db):
        """untag_article() with non-linked tag returns False."""
        from src.storage.sqlite import untag_article

        result = untag_article("some-article-id", "non-linked-tag")
        assert result is False

    def test_get_article_tags_with_tags(self, initialized_db):
        """get_article_tags() for article with tags returns list of tag names."""
        from src.storage.sqlite import add_feed, store_article, tag_article, get_article_tags
        from src.models import Feed

        feed = Feed(
            id="get-tags-feed",
            name="Get Tags Feed",
            url="https://example.com/get-tags.xml",
            etag=None,
            last_modified=None,
            last_fetched=None,
            created_at="2024-01-01T00:00:00+00:00",
        ...
        article_id = store_article(
            guid="get-tags-guid",
            title="Get Tags Article",
            content="Content",
            link="https://example.com/gt",
            feed_id="get-tags-feed",
            pub_date="2024-01-15T10:00:00+00:00",
        )

        tag_article(article_id, "python")
        tag_article(article_id, "testing")
        tag_article(article_id, "tutorial")
    ...
    def test_get_article_tags_without_tags(self, initialized_db):
        """get_article_tags() for article without tags returns empty list."""
        from src.storage.sqlite import add_feed, store_article, get_article_tags
        from src.models import Feed

        feed = Feed(
            id="no-tags-feed",
            name="No Tags Feed",
            url="https://example.com/no-tags.xml",
            etag=None,
            last_modified=None,
            last_fetched=None,
            created_at="2024-01-01T00:00:00+00:00",
        ...
        article_id = store_article(
            ...
            content="Content",
            link="https://example.com/nt",
            feed_id="no-tags-feed",
            pub_date="2024-01-15T10:00:00+00:00",
        )
